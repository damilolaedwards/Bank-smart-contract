...
describe("Bank contract", async function () {
  ...
  it("allows owner deposit rewards", async function () {
    ...
    await mockToken.connect(contractOwner).approve(bankContract.address, reward)
  ...
  it("reverts if non owner deposits reward", async function () {
    ...
    await expect(bankContract.connect(user1).depositReward(reward)).to.be.revertedWith("Ownable: caller is not the owner")
  })

  it("reverts if owner deposits rewards after deposit period", async function () {
    await mockToken.connect(contractOwner).approve(bankContract.address, reward)
    await ethers.provider.send("evm_increaseTime", [86400])
    await expect(bankContract.connect(contractOwner).depositReward(reward)).to.be.revertedWith("Deposit period expired")  
  })
  ...
  it("reverts if user deposit after deposit period", async function () {
    await mockToken.connect(user1).approve(bankContract.address, "100000")
    await ethers.provider.send("evm_increaseTime", [86400])
    await expect(bankContract.connect(user1).deposit("100000")).to.be.revertedWith("Deposit period expired")
  })

  it("reverts if user deposit zero tokens", async function () {
    await mockToken.connect(user1).approve(bankContract.address, "0")
    await expect(bankContract.connect(user1).deposit("0")).to.be.revertedWith("Amount must be greater than zero")
  })
  ...
  it("allows user withdraw stake before deposit window closed", async function () {
    ...
    expect(await mockToken.balanceOf(user1.address)).to.equal(balance.add("100000"))
  })
  ...
  it("reverts if user withdraw after deposit period & before withdrawal period", async function () {
    ...
    await bankContract.connect(user1).deposit("100000")
    await ethers.provider.send("evm_increaseTime", [86400])
    await expect(bankContract.connect(user1).withdraw()).to.be.revertedWith("Withdrawal unavailable")
  })
  ...
  it("allows users collect proportional rewards", async function () {
    ...
    expect(await mockToken.balanceOf(user1.address)).to.equal(user1Balance.add("40000"))
    ...
    expect(await mockToken.balanceOf(user2.address)).to.equal(user2Balance.add("460000"))
    ...
    await expect(bankContract.connect(contractOwner).withdrawReward()).to.be.revertedWith("Can't withdraw rewards yet")

    await ethers.provider.send("evm_increaseTime", [86400])

    await bankContract.connect(contractOwner).withdrawReward()

    expect(await mockToken.balanceOf(contractOwner.address)).to.equal(ownerBalance.add("500000"))
  ...
  it("reverts in non owner tried to withdraw rewards", async function () {
    ...
    await expect(bankContract.connect(user1).withdrawReward()).to.be.revertedWith("Ownable: caller is not the owner")
  })
  ...
  it("it allows all rewards to be distributed", async function () {
    ...
    await expect(bankContract.connect(contractOwner).withdrawReward()).to.be.revertedWith("Users funds still in pool")

    await bankContract.connect(user3).withdraw()

    expect(await mockToken.balanceOf(user1.address)).to.equal(user1Balance.add("20000"))

    expect(await mockToken.balanceOf(user2.address)).to.equal(user2Balance.add("213333"))

    expect(await mockToken.balanceOf(user3.address)).to.equal(user3Balance.add("766667"))

    await expect(bankContract.connect(contractOwner).withdrawReward()).to.be.revertedWith("Rewards pool empty")
